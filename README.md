...
如果部署在公司本地服务器上，`start.bat` 窗口会同时显示局域网访问地址，例如：

```text
http://192.168.1.25:8787
```

其他同事在同一公司网络内用这个局域网地址访问即可。运行期间不要关闭 `start.bat` 窗口。
...
1. 启动 `start.bat`。
2. 服务器本机打开 `http://127.0.0.1:8787`；其他同事打开启动窗口里显示的局域网地址。
...
公司内网部署也只使用 `start.bat`：

1. 在服务器电脑上安装 Node.js 20 或更高版本。
2. 从 GitHub 下载项目后，在项目目录执行一次 `npm install`。
3. 双击 `start.bat` 启动程序。
4. 管理员可在服务器本机导入或更新二维码索引。
5. 其他同事使用启动窗口里的 `LAN` 地址访问，并只使用上传破损二维码和匹配功能。
...
- `PORT=8787`：默认端口。若端口被占用，可在 `start.bat` 中改成其他端口，例如 `8788`。

页面上的“导入清晰二维码索引”板块仍会显示。普通查询人员不要使用该板块，避免误触发服务器本机路径导入。
